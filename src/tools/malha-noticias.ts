...
import type { Noticia, ToolResponse } from "../types.js";

export function registerMalhaNoticias(server: McpServer): void {
...
Exemplos:
  ...
  - "Contorno de Manaus" → tipo="municipio", municipio_id="1302603"`,
      inputSchema: z.object({
        tipo: z.enum(["estado", "municipios_de_estado", "municipio"])
        ...
        formato: z.enum(["svg", "json", "topojson"]).default("json")
          .describe("Formato de saída: 'json'=GeoJSON, 'topojson'=TopoJSON, 'svg'=SVG"),
        resolucao: z.number().int().min(1).max(4).default(2)
          .describe("Resolução: 1=baixa, 2=média, 3=alta, 4=máxima"),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    },
    async ({ tipo, uf, municipio_id, formato, resolucao }): Promise<ToolResponse> => {
      try {
        let url: string;
        ...
        switch (tipo) {
          case "estado":
            if (!uf) return { content: [{ type: "text", text: "Erro: uf é obrigatório para tipo='estado'" }] };
            url = `${IBGE_API.v2}/malhas/estados/${uf}?formato=${formato}&resolucao=${resolucao}`;
            descricao = `Malha do estado ${uf}`;
            break;

          case "municipios_de_estado":
            if (!uf) return { content: [{ type: "text", text: "Erro: uf é obrigatório para tipo='municipios_de_estado'" }] };
            url = `${IBGE_API.v2}/malhas/estados/${uf}/municipios?formato=${formato}&resolucao=${resolucao}`;
            descricao = `Malha dos municípios de ${uf}`;
            break;

          case "municipio":
            if (!municipio_id) return { content: [{ type: "text", text: "Erro: municipio_id é obrigatório para tipo='municipio'" }] };
            url = `${IBGE_API.v2}/malhas/municipios/${municipio_id}?formato=${formato}&resolucao=${resolucao}`;
...
Exemplos:
  ...
  - "Releases do IPCA" → produto="IPCA"`,
    ...
    async ({ quantidade, assunto, produto }): Promise<ToolResponse> => {
      try {
        let url = `${IBGE_API.v3}/noticias/?quantidade=${quantidade}`;
        if (assunto) url += `&busca=${encodeURIComponent(assunto)}`;
        if (produto) url += `&produto=${encodeURIComponent(produto)}`;

        const resposta = await ibgeFetch<{ total: number; itens: Noticia[] }>(url);
        ...
        return {
          content: [{
            ...
            text: truncateIfNeeded(
              `**Notícias do IBGE** (${noticias.length} de ${resposta.total} total)\n\n${texto}`
            ),
          }],
        };
      } catch (err) {
...
interface SidraAgregadoDetalhe {
  id: string;
  nome: string;
  pesquisaID: string;
  pesquisaNome: string;
