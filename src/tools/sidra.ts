...
Retorna: id, nome, período e variáveis das tabelas encontradas (máx. 50).`,
    ...
    async ({ pesquisa_id, nome_filtro }): Promise<ToolResponse> => {
      try {
        ...
        const agregados = await ibgeFetch<SidraAgregado[]>(url);
        let lista = agregados;
        if (nome_filtro) {
        ...
        return { content: [{ type: "text", text: truncateIfNeeded(`**Tabelas SIDRA: ${lista.length} encontradas**\n\n${texto}${aviso}`) }] };
      } catch (err) {
...
Args:
  - tabela_id (string): código da tabela (ex: "9514", "1419", "5938")`,
      inputSchema: z.object({
        tabela_id: z.string().describe("Código da tabela SIDRA (ex: '9514', '1419', '5938')"),
      }),
    ...
    async ({ tabela_id }): Promise<ToolResponse> => {
      try {
        interface Meta {
          ...
          classificacoes: Array<{ id: string; nome: string; categorias: Array<{ id: string; nome: string }> }>;
        }
...
Tabelas populares: 9514=Censo2022, 5938=PIB Municipal, 1419=IPCA, 6579=PNAD Desocupação`,
      inputSchema: z.object({
        ...
        variavel: z.string().optional().describe("Código da variável (sem valor = todas)"),
    ...
    async ({ tabela_id, variavel, localidade, periodo, classificacao }): Promise<ToolResponse> => {
      try {
...
Retorna: série histórica com variação percentual por mês.`,
    ...
    async ({ ultimos_meses, tipo }): Promise<ToolResponse> => {
      try {
        const varMap: Record<string, string> = { variacao_mensal: "63", variacao_acumulada_ano: "69", variacao_acumulada_12_meses: "2266" };
        const url = `${IBGE_API.v3}/agregados/1419/periodos/last%20${ultimos_meses}/variaveis/${varMap[tipo]}?localidades=N1[all]`;
        const dados = await ibgeFetch<SidraResultado[]>(url);
...
Retorna: PIB em mil reais e PIB per capita.`,
    ...
    async ({ municipio_id, estado_id, ano }): Promise<ToolResponse> => {
      try {
        ...
        else if (estado_id) localidade = `N6[in N3[${estado_id}]]`;
      ...
      try {
        ...
        switch (nivel) {
          ...
          case "estado_municipios":
            ...
            localidade = `N6[in N3[${estado_id}]]`; break;
  ...
  - "Ranking das capitais" → sem estado_id`,
    ...
    async ({ estado_id, top_n, ordenar }): Promise<ToolResponse> => {
      ...
      try {
        ...
        const url = `${IBGE_API.v3}/agregados/9543/periodos/2022/variaveis/2513?localidades=${localidade}&view=flat`;
        const dados = await ibgeFetch<SidraResultado[]>(url);
