...
import type { SidraAgregado, SidraResultado, ToolResponse } from "../types.js";

const LOCALIDADES_COMUNS: Record<string, string> = {
  BR: "N1[all]",
  REGIOES: "N2[all]",
  ESTADOS: "N3[all]",
  MUNICIPIOS: "N6[all]",
...
const CATALOGO_TEMAS: Array<{ temas: string[]; tabelas: Array<{ id: string; nome: string; variavel?: string }> }> = [
  {
...
function formatarSerie(serie: Record<string, string>): string {
  return Object.entries(serie).map(([p, v]) => `  ${p}: ${v}`).join("\n");
}

function formatarResultadoSidra(dados: SidraResultado[]): string {
  const linhas: string[] = [];
...
function resolverLocalidade(localidade: string): string {
  if (LOCALIDADES_COMUNS[localidade]) return LOCALIDADES_COMUNS[localidade];
  if (/^\d{2}$/.test(localidade)) return `N3[${localidade}]`;
  if (/^\d{7}$/.test(localidade)) return `N6[${localidade}]`;
  return "N1[all]";
}

export function registerSidraTools(server: McpServer): void {
...
Retorna: id, nome, período e variáveis das tabelas encontradas (máx. 50).`,
      inputSchema: z.object({
        pesquisa_id: z.string().optional().describe("Código da pesquisa IBGE (ex: CN, CA, EC, IN, PD, PA)"),
        nome_filtro: z.string().optional().describe("Texto para filtrar tabelas pelo nome"),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    },
    async ({ pesquisa_id, nome_filtro }): Promise<ToolResponse> => {
      try {
        ...
        if (nome_filtro && !pesquisa_id) {
          ...
          const matches = CATALOGO_TEMAS.filter((entrada) =>
            entrada.temas.some((t) => {
              const tNorm = t.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
              return tNorm.includes(termo) || termo.includes(tNorm);
            })
          );
          if (matches.length > 0) {
            const linhas = matches.flatMap((m) =>
              m.tabelas.map((t) =>
                `• **Tabela ${t.id}** — ${t.nome}${t.variavel ? `\n  Variável principal: ${t.variavel}` : ""}`
              )
            );
            return {
              content: [{
                type: "text", text:
                  `**Tabelas IBGE para "${nome_filtro}"**\n\n${linhas.join("\n\n")}\n\n` +
        ...
        // 2. Fallback: busca na API SIDRA
        // ATENÇÃO: A API retorna GRUPOS por pesquisa: [{id:"CN", nome:"Censo", agregados:[...]}]
        // É necessário fazer flatMap para obter a lista real de tabelas
        interface PesquisaGrupo { id: string; nome: string; agregados: SidraAgregado[] }
        const url = pesquisa_id
          ? `${IBGE_API.v3}/agregados?pesquisa=${pesquisa_id}`
          : `${IBGE_API.v3}/agregados`;
        const grupos = await ibgeFetch<PesquisaGrupo[]>(url);

        // Aplanar grupos → tabelas reais, preservando pesquisaID/Nome
        let lista: SidraAgregado[] = grupos.flatMap((g) =>
          (g.agregados ?? []).map((a) => ({
            ...a,
        ...
        return {
          content: [{
            ...
            text: truncateIfNeeded(
              `**Tabelas SIDRA: ${lista.length} encontradas** (pesquisa: ${pesquisa_id ?? "todas"})\n\n${texto}${aviso}`
            )
          }]
        };
      } catch (err) {
...
Args:
  - tabela_id (string): código da tabela (ex: "9514", "1419", "5938", "9543")`,
      inputSchema: z.object({
        tabela_id: z.string().describe("Código da tabela SIDRA"),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    },
    async ({ tabela_id }): Promise<ToolResponse> => {
      try {
        interface Categoria { id: string; nome: string; unidade: string }
        interface Classificacao { id: string; nome: string; sumarizacao: string[]; categorias: Categoria[] }
        interface Meta {
          id: string; nome: string; pesquisaID: string; pesquisaNome: string;
        ...
        const dados = await ibgeFetch<Meta>(`${IBGE_API.v3}/agregados/${tabela_id}/metadados`);
...
Retorna: lista de períodos no formato AAAA, AAAAMM ou trimestre.`,
      inputSchema: z.object({
        tabela_id: z.string().describe("Código da tabela SIDRA (ex: '1419', '9514', '6381')"),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    },
    async ({ tabela_id }): Promise<ToolResponse> => {
      try {
        interface Periodo { id: string; literals: string[]; modificadores: { id: number; nome: string }[] }
        const periodos = await ibgeFetch<Periodo[]>(`${IBGE_API.v3}/agregados/${tabela_id}/periodos`);
        if (!periodos?.length) {
...
Tabelas populares: 9514=Censo2022 pop, 9543=Censo2022 alfabetização, 5938=PIB Municipal, 1419=IPCA, 6381=PNAD Desocupação`,
      inputSchema: z.object({
        tabela_id: z.string().describe("Código da tabela SIDRA"),
        variaveis: z.string().optional().describe("Código(s) da(s) variável(is) separados por pipe '|' (ex: '93|94|95')"),
        localidade: z.string().default("BR").describe("BR, ESTADOS, REGIOES, código de estado (2 dígitos) ou município (7 dígitos)"),
        periodo: z.string().default("last").describe("last, last[N], AAAA, AAAAMM"),
        classificacao: z.string().optional().describe("Filtro de classificação no formato 'ID[cats]'"),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    },
    async ({ tabela_id, variaveis, localidade, periodo, classificacao }): Promise<ToolResponse> => {
      try {
        ...
        let url = `${IBGE_API.v3}/agregados/${tabela_id}/periodos/${periodoNorm}/variaveis/${variavelStr}?localidades=${codigoLocalidade}`;
        if (classificacao) url += `&classificacao=${classificacao}`;
        const dados = await ibgeFetch<SidraResultado[]>(url, TTL.SIDRA_DATA);
        if (!dados?.length) return { content: [{ type: "text", text: `Nenhum dado encontrado para a tabela ${tabela_id}.` }] };
        const resultado = formatarResultadoSidra(dados);
        return { content: [{ type: "text", text: truncateIfNeeded(`**SIDRA Tabela ${tabela_id}** | ${localidade} | ${periodo}\n${resultado}`) }] };
      } catch (err) {
        return { content: [{ type: "text", text: formatToolError(err) }] };
      }
    }
...
Retorna: série histórica com variação percentual por mês.`,
      inputSchema: z.object({
        ultimos_meses: z.number().int().min(1).max(60).default(12).describe("Número de meses (1-60)"),
        tipo: z.enum(["variacao_mensal", "variacao_acumulada_ano", "variacao_acumulada_12_meses"])
          .default("variacao_mensal").describe("Tipo de variação"),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: false, openWorldHint: false },
    },
    async ({ ultimos_meses, tipo }): Promise<ToolResponse> => {
      try {
...
Retorna: PIB em mil reais e PIB per capita.`,
      inputSchema: z.object({
        municipio_id: z.string().optional().describe("Código IBGE do município (7 dígitos, ex: '3550308' para São Paulo)"),
        estado_id: z.string().optional().describe("Código IBGE do estado (2 dígitos, ex: '35' para SP)"),
        ano: z.string().optional().describe("Ano de referência (ex: '2021')"),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    },
    async ({ municipio_id, estado_id, ano }): Promise<ToolResponse> => {
      try {
        const periodo = ano ?? "last";
        let localidade: string;
        if (municipio_id) localidade = `N6[${municipio_id}]`;
        else if (estado_id) localidade = `N6[in%20N3[${estado_id}]]`;
        else localidade = "N6[all]";
        const url = `${IBGE_API.v3}/agregados/5938/periodos/${periodo}/variaveis/37|497?localidades=${localidade}`;
        const dados = await ibgeFetch<SidraResultado[]>(url);
        return { content: [{ type: "text", text: truncateIfNeeded(`**PIB dos Municípios** (${periodo})\n${formatarResultadoSidra(dados)}`) }] };
      } catch (err) {
        return { content: [{ type: "text", text: formatToolError(err) }] };
      }
    }
...
Retorna: população residente total pelo Censo 2022.`,
      inputSchema: z.object({
        nivel: z.enum(["BR", "REGIOES", "ESTADOS", "municipio", "estado_municipios"])
          .describe("Nível territorial da consulta"),
        municipio_id: z.string().optional().describe("Código IBGE do município (7 dígitos)"),
        estado_id: z.string().optional().describe("Código IBGE do estado (2 dígitos)"),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    },
    async ({ nivel, municipio_id, estado_id }): Promise<ToolResponse> => {
      try {
        let localidade: string;
        switch (nivel) {
          case "BR": localidade = "N1[all]"; break;
          case "REGIOES": localidade = "N2[all]"; break;
          case "ESTADOS": localidade = "N3[all]"; break;
          case "municipio":
            if (!municipio_id) return { content: [{ type: "text", text: "Erro: municipio_id obrigatório quando nivel='municipio'" }] };
            localidade = `N6[${municipio_id}]`; break;
          case "estado_municipios":
            if (!estado_id) return { content: [{ type: "text", text: "Erro: estado_id obrigatório quando nivel='estado_municipios'" }] };
            localidade = `N6[in%20N3[${estado_id}]]`; break;
          default: localidade = "N1[all]";
        }
        const url = `${IBGE_API.v3}/agregados/9514/periodos/2022/variaveis/93?localidades=${localidade}`;
        const dados = await ibgeFetch<SidraResultado[]>(url);
        return { content: [{ type: "text", text: truncateIfNeeded(`**População — Censo 2022** | ${nivel}\n${formatarResultadoSidra(dados)}`) }] };
      } catch (err) {
        return { content: [{ type: "text", text: formatToolError(err) }] };
      }
    }
...
Exemplos:
  ...
  - "Ranking das capitais" → sem estado_id`,
      inputSchema: z.object({
        estado_id: z.string().regex(/^\d{2}$/).optional()
        ...
        ordenar: z.enum(["maior", "menor"]).default("maior")
          .describe("maior = melhor taxa primeiro | menor = pior taxa primeiro"),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    },
    async ({ estado_id, top_n, ordenar }): Promise<ToolResponse> => {
      ...
      try {
        ...
        const url = `${IBGE_API.v3}/agregados/9543/periodos/2022/variaveis/2513?localidades=${localidade}`;
        const dados = await ibgeFetch<SidraResultado[]>(url);
        ...
        interface MunAlfa { id: string; nome: string; taxa: number }
        const municipios: MunAlfa[] = series
          .map((s) => ({
            id: s.localidade.id,
            nome: s.localidade.nome,
            taxa: parseFloat((Object.values(s.serie)[0] as string) ?? "0"),
          }))
          .filter((m) => m.taxa > 0);
...
Exemplo: "Qual o PIB de São Paulo em 2021?" ou "Ranking de PIB dos estados"`,
      inputSchema: z.object({
        ano: z.number().int().min(1995).max(2030).describe("Ano do PIB (ex: 2021)"),
        uf_id: z.number().int().positive().optional().describe("ID do estado (ex: 35=SP). Omita para todos."),
        per_capita: z.boolean().default(false).describe("true=PIB per capita, false=PIB total"),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    },
    async ({ ano, uf_id, per_capita }) => {
      try {
        ...
        const url = `${IBGE_API.v3}/agregados/5938/periodos/${ano}/variaveis/${variavel}?localidades=${localidade}&view=flat`;
        const dados = await ibgeFetch<SidraResultado[]>(url, TTL.SIDRA_DATA);
        ...
        const estados: EstadoPib[] = series.map((s) => ({
          nome: s.localidade.nome,
          id: s.localidade.id,
          valor: parseFloat((Object.values(s.serie)[0] as string) ?? "0"),
        })).filter((e) => e.valor > 0);
        ...
        return {
          content: [{
            type: "text",
            text: truncateIfNeeded(
              `## ${titulo} por Estado — ${ano}${totalFormatado}\n\n` +
              linhas.join("\n") +
