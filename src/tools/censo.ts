// tools/censo.ts — Dados do Censo Demográfico 2022
...
function getSerie(dados: SidraResultado[]): Array<{ nome: string; id: string; valor: number }> {
...
export function registerCensoTools(server: McpServer): void {
...
Use ibge_listar_municipios para obter o ID de qualquer município.`,
      inputSchema: z.object({
        municipio_id: z.number().int().positive().describe("Código IBGE do município (7 dígitos)"),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    },
    async ({ municipio_id }) => {
      try {
        const url = `${IBGE_API.v3}/agregados/9514/periodos/2022/variaveis/93?localidades=N6[${municipio_id}]`;
        ...
        const pop = parseFloat(Object.values(serie.serie)[0] as string);
        return {
          content: [{
            type: "text", text:
              `## ${serie.localidade.nome} — Censo 2022\n\n**População Total:** ${formatNum(pop)} habitantes\n\n_Fonte: IBGE Censo 2022 (Tabela 9514)_`
          }]
        };
      } catch (err) {
...
Retorna população, área em km² e densidade por estado, ordenados por densidade.`,
      inputSchema: z.object({
        uf_id: z.number().int().positive().optional().describe("ID do estado. Omita para todos."),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    },
    async ({ uf_id }) => {
      try {
        ...
        const area = getSerie(dadosArea as SidraResultado[]);
