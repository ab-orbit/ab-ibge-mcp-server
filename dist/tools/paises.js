...
exports.registerPaisesTools = registerPaisesTools;
 ...
 */
function registerPaisesTools(server) {
    // ─── Lista todos os países ─────────────────────────────────────────────────
    server.registerTool("ibge_listar_paises", {
        title: "Listar todos os países",
        description: "Retorna lista de todos os países do mundo com informações básicas (nome, código ISO, região). " +
        ...
        try {
            const url = `${ibge_client_js_1.IBGE_API.v1}/paises?orderBy=${orderBy}`;
            const paises = await (0, ibge_client_js_1.ibgeFetch)(url);
            ...
            return { content: [{ type: "text", text: (0, ibge_client_js_1.truncateIfNeeded)(texto) }] };
        }
        catch (err) {
    ...
    server.registerTool("ibge_obter_pais", {
        title: "Obter informações detalhadas de um país",
        description: "Retorna informações completas sobre um país específico usando código ISO-3166-1-ALPHA-2 (ex: BR, US, PT). " +
        ...
        try {
            const url = `${ibge_client_js_1.IBGE_API.v1}/paises/${codigo_iso}`;
            const resultado = await (0, ibge_client_js_1.ibgeFetch)(url);
            if (!resultado || resultado.length === 0) {
                return { content: [{ type: "text", text: `❌ País não encontrado com código ISO: ${codigo_iso}` }] };
            }
            const pais = resultado[0];
            const texto = `**${pais.nome.abreviado}** (${pais.id["ISO-3166-1-ALPHA-2"]})\n\n` +
                `📍 **Localização**\n` +
    ...
    server.registerTool("ibge_buscar_pais", {
        title: "Buscar países por nome",
        description: "Busca países cujo nome contenha o termo especificado (busca case-insensitive). " +
        ...
        inputSchema: zod_1.z.object({
            nome: zod_1.z.string().min(2).describe("Nome ou parte do nome do país para buscar (mín. 2 caracteres)"),
        }),
        annotations: {
            readOnlyHint: true,
            destructiveHint: false,
        ...
        try {
            const paises = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v1}/paises`);
            const termo = nome.toLowerCase();
            const encontrados = paises.filter((p) => p.nome.abreviado.toLowerCase().includes(termo) ||
            ...
            const texto = `**Países Encontrados** (${encontrados.length} resultado${encontrados.length > 1 ? "s" : ""})\n\n` +
                encontrados
                    .map((p) => `• **${p.nome.abreviado}** (${p.id["ISO-3166-1-ALPHA-2"]} / ${p.id["ISO-3166-1-ALPHA-3"]})\n` +
    ...
    server.registerTool("ibge_listar_indicadores_paises", {
        title: "Listar indicadores disponíveis para países",
        description: "Retorna lista completa dos 34 indicadores disponíveis para países, organizados por categoria. " +
    ...
    }, async ({ categoria = "todos" }) => {
        try {
            const indicadores = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v1}/paises/indicadores`);
    ...
    server.registerTool("ibge_indicadores_pais", {
        title: "Consultar indicadores de um país",
        description: "Retorna séries históricas de um ou mais indicadores para um país específico. " +
        ...
        inputSchema: zod_1.z.object({
            codigo_iso: zod_1.z.string().length(2).toUpperCase().describe("Código ISO-3166-1-ALPHA-2 do país (2 letras)"),
            indicadores: zod_1.z
                .string()
    ...
    }, async ({ codigo_iso, indicadores }) => {
        try {
            // Encode pipe character for URL
            const indicadoresEncoded = indicadores.replace(/\|/g, "%7C");
            const url = `${ibge_client_js_1.IBGE_API.v1}/paises/${codigo_iso}/indicadores/${indicadoresEncoded}`;
    ...
    server.registerTool("ibge_comparar_paises", {
        title: "Comparar indicadores entre múltiplos países",
        description: "Compara um indicador específico entre vários países, mostrando os valores mais recentes. " +
            ...
            "  • indicador_id: '77823'",
        inputSchema: zod_1.z.object({
            codigos_iso: zod_1.z.string().describe("Códigos ISO-3166-1-ALPHA-2 separados por | (ex: 'BR|AR|US|PT')"),
            indicador_id: zod_1.z.string().describe("ID do indicador a comparar (ex: 77823 para PIB per capita)"),
        }),
        annotations: {
            readOnlyHint: true,
            destructiveHint: false,
    ...
    }, async ({ codigos_iso, indicador_id }) => {
        try {
            const paises = codigos_iso.split("|");
            const resultados = [];
            for (const codigo of paises) {
                const url = `${ibge_client_js_1.IBGE_API.v1}/paises/${codigo.trim()}/indicadores/${indicador_id}`;
                const dados = await (0, ibge_client_js_1.ibgeFetch)(url);
                if (!dados || dados.length === 0)
                    continue;
            ...
            const indicadorInfo = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v1}/paises/indicadores`);
