...
exports.registerCnaeTools = registerCnaeTools;
...
function formatarCaminhoHierarquico(sub) {
...
// ─── Registro das Tools ──────────────────────────────────────────────────────
function registerCnaeTools(server) {
    // ── Tool 1: busca textual ──────────────────────────────────────────────────
    server.registerTool("ibge_cnae_buscar", {
...
Seções principais: A=Agro, B=Mineração, C=Indústria, D=Energia, E=Saneamento,
  ...
  P=Educação, Q=Saúde, R=Arte/Cultura, S=Outros Serviços, T=Doméstico, U=Organismos Int'l`,
        inputSchema: zod_1.z.object({
            termo: zod_1.z.string().min(2).describe("Palavra-chave para buscar (ex: 'padaria', 'software', 'engenharia')"),
            secao_id: zod_1.z.string().length(1).toUpperCase().optional()
                .describe("Letra da seção CNAE para filtrar (ex: 'C' para indústria, 'G' para comércio)"),
            max_resultados: zod_1.z.number().int().min(1).max(50).default(10)
                .describe("Máximo de resultados a retornar (padrão 10)"),
        }),
        annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    }, async ({ termo, secao_id, max_resultados }) => {
        try {
            const subclasses = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v2}/cnae/subclasses`);
            const termoNorm = normalizar(termo);
            const termos = termoNorm.split(/\s+/).filter((t) => t.length >= 2);
            const resultados = subclasses.filter((sub) => {
                // Filtro por seção
                if (secao_id && sub.classe.grupo.divisao.secao.id !== secao_id.toUpperCase())
                    return false;
            ...
            if (resultados.length === 0) {
                ...
                return {
                    content: [{
                            type: "text",
                            text: (0, ibge_client_js_1.truncateIfNeeded)(`**CNAE — Resultados para "${termo}"** (busca ampliada — ${resultadosOR.length} encontrados, mostrando ${recorte.length})\n\n${linhas}`)
                        }]
                };
            }
            ...
            return {
                content: [{
                        type: "text",
                        text: (0, ibge_client_js_1.truncateIfNeeded)(`**CNAE — "${termo}"** — ${resultados.length} atividade(s) encontrada(s)\n\n${linhas}${aviso}`)
                    }]
            };
        }
        catch (err) {
    ...
    server.registerTool("ibge_cnae_detalhar", {
...
Retorna: nome, hierarquia completa e, para classes/subclasses, as atividades incluídas.`,
        inputSchema: zod_1.z.object({
            codigo: zod_1.z.string().min(1).max(10).describe("Código CNAE: seção ('G'), divisão ('47'), grupo ('47.1'), classe ('4711-3') ou subclasse ('4711-3/02')"),
        }),
        annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    }, async ({ codigo }) => {
        try {
            ...
            if (/^[A-U]$/.test(cod))
                ...
                nivel = "divisao";
            else if (/^\d{2}\.\d$/.test(cod))
                nivel = "grupo";
            else if (/^\d{4}-\d$/.test(cod))
                nivel = "classe";
            else if (/^\d{4}-\d\/\d{2}$/.test(cod))
                nivel = "subclasse";
            else {
                return {
                    content: [{
            ...
            // Seção
            if (nivel === "secao") {
                const secao = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v2}/cnae/secoes/${cod}`);
                const divisoes = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v2}/cnae/secoes/${cod}/divisoes`);
            ...
            // Divisão
            if (nivel === "divisao") {
                const divisao = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v2}/cnae/divisoes/${cod}`);
                const grupos = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v2}/cnae/divisoes/${cod}/grupos`);
                const secNome = divisao.secao?.nome ?? "—";
            ...
            // Grupo
            if (nivel === "grupo") {
                const grupo = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v2}/cnae/grupos/${cod}`);
                const classes = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v2}/cnae/grupos/${cod}/classes`);
                const linhasClasses = classes.map((c) => `  ${c.id} — ${c.nome}`).join("\n");
                const divNome = grupo.divisao?.nome ?? "—";
                return {
                    content: [{
                            type: "text",
                            text: [
            ...
            // Classe
            if (nivel === "classe") {
                const classe = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v2}/cnae/classes/${cod}`);
                const subclasses = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v2}/cnae/classes/${cod}/subclasses`);
                const linhasSubs = subclasses.map((s) => `  ${s.id} — ${s.descricao}${s.observacoes ? `\n    ⚠️ ${s.observacoes}` : ""}`).join("\n");
                const grupoNome = classe.grupo?.nome ?? "—";
                return {
                    content: [{
                            type: "text",
                            text: [
            ...
            const todasSubs = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v2}/cnae/subclasses`);
    ...
    server.registerTool("ibge_cnae_secoes", {
...
Seções: A(Agro) B(Mineração) C(Indústria) D(Energia) E(Saneamento) F(Construção)
  ...
  R(Arte/Esporte) S(Outros Serviços) T(Doméstico) U(Organismos Internacionais)`,
        ...
        try {
            const secoes = await (0, ibge_client_js_1.ibgeFetch)(`${ibge_client_js_1.IBGE_API.v2}/cnae/secoes`);
